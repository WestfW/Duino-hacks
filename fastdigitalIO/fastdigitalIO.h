 ...
 *    done at compile time.  Feature 3) with LTO, the compiler can do that
 *    constant optimization on arrays in PROGMEM that are actually defined
 *    in other modules.
 *
 * Used together, this means that a digitalWriteFast() inline function used
 * in the main user sketch can access the same PROGMEM tables that the
 * normal digitalWrite() function uses to do the port/mask translation.  If
 * all the values involved are constants, code that looks very like the
 * normal digitalWrite() function will end up producing the single
 * bitset/bitclear instructions (assuming that the ports are withing
 * range.)
 *
 ...
 * always fail.  If LTO is not enabled, the compiler will include a
 * function call with the "error" attribute, which will result in the
